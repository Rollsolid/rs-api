...
def sample_flop_and_player_hands(all_cards=None, n_players=5, n_flops=0, flops=None):
    ...
    if flops:
        ...
        flop = [all_cards.pop(random.randint(0, len(all_cards)-1)) for _ in range(n_flops)]
    return hands, flop
...
def generate_game(n_other_players, n_flops, my_hand, flops):
    all_cards = get_deck(my_hand)
    if my_hand:
        player_hand = my_hand
    else:
        player_hand = [all_cards.pop(random.randint(0, len(all_cards)-1)) for _ in range(2)]
    hands, flops = sample_flop_and_player_hands(all_cards,n_other_players , n_flops, flops=flops)
    res = eval_hands_QUICK(player_hand, hands, flops)
    return my_hand, res
    return {   "player_hand": player_hand,
    "other_hands": hands,
    "flops":     flops,
    "results": res
    }
...
for x in range(iterations):
    ...
    player_hand, res = generate_game(n_other_players=5, n_flops=0, my_hand = my_hand, flops=flops)
    ...
    try:
        ...
        total_games = deck_hash_perf[player_hand]["wins"] + deck_hash_perf[player_hand]["losses"] + deck_hash_perf[player_hand]["draws"]
